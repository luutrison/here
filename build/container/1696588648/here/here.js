        ...
        MAP_PATH: ({ path, subPath }) => {
            try {
                ...
                if (arrSubPath[NORMAL.NUMBER_ZERO] != NORMAL.NGA_SYMBOL && !isAbsolute) {
                    ...
                    if (RELEASE) {
                        RELEASE_PATH = RELEASE + NORMAL.RIGHT_SLASH_SYMBOL
                    }

                    const arrSubPathe = arrSubPath.join(NORMAL.STRING_EMPTY)
                    PATH_RETURN = RELEASE_PATH + THIS.CONVERT.PATH_HEAD_EMPTY({
        ...
        THIS: {
            GET: () => {
                try {
                    ...
                    if (!THIS.CHECK.IS_ARRAY_EMPTY({ ob: element })) {
                        const data = JSON.parse(atob($(element).attr(THIS.OPTIONS().ATTRIBUTE.DATA)))
                        THIS.CURRENT.PROPERTIES.STATUS = data
                    }
                } catch (err) { THIS.EX.ERROR({ err: err }) }
            }
        },
